...
$patchLocation = "C:\Deploy\Patches\"
$patchfile = Get-ChildItem -Path "$patchLocation" | where{$_.Extension -eq ".exe"}
$patchPath = $patchLocation + $patchfile
...
####################################
##Stop IISAdmin and Timer Services##
####################################
...
$filename = $patchPath.BaseName    
Start-Process $patchPath
